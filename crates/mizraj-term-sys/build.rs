...
fn emit_link_flags_macos() {
    let Some(dir) = env::var_os("LIBGHOSTTY_LIB_DIR").map(PathBuf::from) else {
        println!(
            "cargo:warning=LIBGHOSTTY_LIB_DIR is not set - downstream binaries that use mizraj-term-sys will fail to link; export it to the directory containing libghostty.dylib"
        );
        return;
    };
...
fn emit_link_flags_linux() {
    let Some(dir) = env::var_os("LIBGHOSTTY_LIB_DIR").map(PathBuf::from) else {
        println!(
            "cargo:warning=LIBGHOSTTY_LIB_DIR is not set - downstream binaries that use mizraj-term-sys will fail to link; export it to the directory containing libghostty.so"
        );
        return;
    };
