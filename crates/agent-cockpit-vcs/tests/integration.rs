...
use agent_cockpit_vcs::git2;
use agent_cockpit_vcs::{
    current_branch, diff_head_base, diff_session, diff_working_tree, repo_open, worktree_list,
};
...
fn seed_fixture() -> Fixture {
    ...
    let setup = Path::new(env!("CARGO_MANIFEST_DIR"))
        .join("tests/fixtures/sample-repo/setup.sh");
...
fn end_to_end_vcs_api_on_seeded_fixture() {
    ...
    let branch = current_branch(&repo).expect("current_branch");
    ...
    assert_eq!(worktrees[0].branch, "locked");
