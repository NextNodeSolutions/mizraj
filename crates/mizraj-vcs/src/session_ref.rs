...
const REFLOG_MSG: &str = "mizraj session ref";

/// Create `refs/mizraj/sessions/<session_id>` pointing at the current
/// HEAD commit.
///
...
mod tests {
    ...
    fn creates_ref_at_head() {
        ...
        let r = repo
            .find_reference("refs/mizraj/sessions/01HX0000000000000000000000")
    ...
    fn ref_is_listed_under_session_prefix() {
        ...
        let mut names: Vec<String> = repo
            .references_glob("refs/mizraj/sessions/*")
            .expect("glob")
            .names()
            .filter_map(|n| n.ok().map(|s| s.to_string()))
            .collect();
        names.sort();
        assert_eq!(names, vec!["refs/mizraj/sessions/abc".to_string()]);
    }
