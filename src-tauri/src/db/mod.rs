...
use agent_cockpit_vcs::{origin_url, repo_open};
...
mod tests {
    ...
    fn slug_from_https_url_strips_dot_git() {
        assert_eq!(
            slug_from_remote_url("https://github.com/NextNodeSolutions/agent-cockpit.git"),
            Some("agent-cockpit".to_string())
        );
    }
    ...
    fn slug_from_scp_url_strips_dot_git() {
        assert_eq!(
            slug_from_remote_url("git@github.com:NextNodeSolutions/agent-cockpit.git"),
            Some("agent-cockpit".to_string())
        );
    }
    ...
    fn progress_db_path_lives_under_home_mizraj_slug() {
        let path = progress_db_path("agent-cockpit");
        assert!(path.ends_with("mizraj/agent-cockpit/progress.db"));
    }
