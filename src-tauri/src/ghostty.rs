//! Bridge between the Tauri host and the `mizraj-config` crate.
//!
...
//! effective config, and maps it to a serde wire DTO the frontend renderer
//! consumes. Per-cell palette resolution and runtime color injection into
//! libghostty (OSC) are a later track; this command delivers the *config* the
//! renderer keys off (font, cursor, padding, colors, palette overrides).

use std::path::{Path, PathBuf};

use mizraj_config::{
    load, Adjustment, Appearance, Color, CopyOnSelect, CursorStyle, Diagnostic, LoadOptions,
    PaddingAxis, ResolvedConfig,
};
use serde::Serialize;
...
fn build_dto(config: ResolvedConfig) -> GhosttyConfigDto {
