...
mod tests {
    ...
    fn returns_entries_for_interview_and_plan_artifacts() {
        ...
        fs::write(
            plans_dir.join("2026-05-15-mizraj.html"),
    ...
    fn extract_title_handles_real_plan_html_fixture() {
        let html = include_str!("../../../docs/plans/2026-05-16-mizraj-backlog.html");
        let title = extract_title(html, "2026-05-16-mizraj-backlog");
        assert_eq!(title, "Mizraj — Backlog V1 (106 tâches)");
    }
    ...
    fn list_plans_populates_title_from_real_fixture() {
        ...
        let entries = collect_entries(tmp.path()).expect("collect_entries");
        assert_eq!(entries.len(), 1);
        assert_eq!(entries[0].title, "Mizraj — Backlog V1 (106 tâches)");
    }
